...
def main():
    ...
    if page == "Global Stock 360°":
        ...
        if st.button("Run Full Institutional Analysis"):
            with st.spinner("Processing Market Data..."):
                ...
                if not stock_data.empty:
                    ...
                    fig_perf.update_layout(title="Strategic Performance Tracking", template="plotly_dark")
                    ...
                    fig_mc.update_layout(title="GBM Potential Price Paths", template="plotly_dark")
        ...
        if st.button("Generate Forecast"):
            ...
            fig_f.update_layout(title=f"30-Day Trend Analysis for {symbol}", template="plotly_dark")
        ...
        if uploaded:
            try:
                ...
                if 'Description' in df_user.columns and 'Amount' in df_user.columns:
                    ...
                    if shopping_pct > 30:
                        st.warning(f"AI Observation: High Shopping Spend detected ({shopping_pct:.1f}%). Consider reallocating to investments.")
                    else:
    ...
    st.sidebar.divider()
    st.sidebar.info("📌 **Defense Tip:** Focus on Alpha and the Sharpe Ratio.")
